...
def datos_exportacion():
        ...
        export_files = [
            "Costos de Exportaciones - EXCELLENCE 2026.xlsx","Costos de Exportaciones - GAP 2026.xlsx",
            "Costos de Exportaciones - QBERRIES 2026.xlsx","Costos de Exportaciones - QBERRIES MAGICA 2026.xlsx"
        ]
        ...
        for file in export_files:
            ...
            senasa_df = senasa_df.rename(columns={"FECHA P.":"FECHA"})
            ...
            materi_df = materi_df.rename(columns={"DESCRIPCION2":"CONCEPTO2"})
...
# ============================================================
# LIMPIEZA DE DATAFRAMES DE COMERCIO EXTERIOR
# ============================================================
...
##DATA BD KG: completar el fundo faltante con la etapa (antes de renombrar)
data_bd_kg["NOM DEL FUNDO"] = data_bd_kg["NOM DEL FUNDO"].fillna(data_bd_kg["ETAPA"])
data_senasa["FUNDO"] = data_senasa["FUNDO"].fillna(data_senasa["ETAPA"])
data_maquila["FUNDO"] = data_maquila["FUNDO"].fillna(data_maquila["ETAPA"])
...
data_materi["KEY UNICO MATERIALES"] = data_materi["FCL"]+""+data_materi["FUNDO"]+""+data_materi["FORMATO_ORACLE"]
data_maquila["KEY UNICO MAQUILA"] = data_maquila["FCL"]+""+data_maquila["FUNDO"]+""+data_maquila["FORMATO_ORACLE"]
data_senasa["KEY UNICO SENASA"] = data_senasa["FCL"]+""+data_senasa["FUNDO"]+""+data_senasa["FORMATO_ORACLE"]
data_operador_log["KEY UNICO OPL"] = data_operador_log["FCL"]+""+data_operador_log["FUNDO"]+""+data_operador_log["FORMATO_ORACLE"]

# ============================================================
# SENASA: separar ESTADO_ZARPE en estado (texto) y fecha (date)
# El F. ZARPE de SENASA mezcla el estado ('EXPORTADO') con la fecha de zarpe
# según el archivo. Se separa por contenido y, como la fecha de zarpe por ID
# es la misma en todas las tablas, se completa desde las otras tablas.
# ============================================================
...
# ========================================================================================================================================================================================================================
st.dataframe(data_bd_kg)
st.dataframe(data_materi)
st.dataframe(data_maquila)
st.dataframe(data_senasa)
st.dataframe(data_operador_log)
# ============================================================
# TABLAS DEL MODELO POWER BI (esquema estrella)
# Dim_Envio (llave = ID) + Dim_Calendario. Las 5 tablas de arriba
# quedan como hechos; conservan sus métricas (KG, IMPORTE, etc.).
# ============================================================
...
def construir_dim_envio(tablas):
    ...
    ###
    dim["VARIEDAD"] = dim["VARIEDAD"].replace("-","SEKOYA POP")
    #dim = dim.drop_duplicates(subset=['ID'])
    # Guarda el DataFrame sin la última fila en una nueva variable
    dim = dim.iloc[:-1]
    return dim
...
FUNDO_A_PRESUPUESTO = {
    ...
    # "CANYON MADEIRA": "CANYON",
    # "<fundo>":        "TARA FARM",
    # "<fundo>":        "BIG BERRIES",
}

def agregar_presupuesto(dim):
    """Fusiona las tasas presupuestadas dentro de Dim_Envio, cruzando por FUNDO."""
    ppto = pd.DataFrame(
...
# --- Guardado local para Power BI (carpeta del proyecto, ignorada por git) ---
...
def _exportar_parquet(df, ruta):
    """Fuerza a texto las columnas object (evita tipos mezclados que rompen
    pyarrow, p.ej. ESTADO_ZARPE con 'EXPORTADO' y fechas en distintos archivos)."""
    df = df.copy()
...
for _nombre, _df in TABLAS_PBI.items():
    _exportar_parquet(_df, os.path.join(CARPETA_PBI, f"{_nombre}.parquet"))
st.success(f"{len(TABLAS_PBI)} tablas guardadas en {CARPETA_PBI}")

st.write("Dim_Envio", dim_envio.shape)
st.dataframe(dim_envio)


st.write(data_bd_kg.shape)
st.dataframe(data_bd_kg)
st.write(data_senasa.shape)
st.dataframe(data_senasa)
st.write(data_maquila.shape)
st.dataframe(data_maquila)
st.write(data_materi.shape)
st.dataframe(data_materi)
st.write(data_operador_log.shape)
st.dataframe(data_operador_log)
