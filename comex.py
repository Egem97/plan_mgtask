...
#data_bd_kg["FCL-FUNDO"] = data_bd_kg["FCL"]+"|"+ data_bd_kg["EMPRESA"]

mask_materi_variedad_exc = data_materi["EMPRESA"] == "EXCELLENCE"
...
st.subheader("☁️ Exportación de parquets (local + OneDrive)")
...
for _nombre, _df in TABLAS_PBI.items():
    ...
    if _ok:
        st.success(f"☁️ {_archivo} subido a OneDrive")
    else:
        st.error(f"❌ No se pudo subir {_archivo} a OneDrive (ver logs)")
st.info(f"{len(TABLAS_PBI)} tablas guardadas en {CARPETA_PBI} y enviadas a OneDrive")
